 ...
 * @version     $Id: shInit.php 1078 2009-09-27 10:43:05Z silianacom-svn $
 */
...
if ($sefConfig->Enabled) {
  ...
  // global flag to allow our various automatic redirections
  // hack to avoid doing redirects in case our router parse() method
  // is called several times. After the first time, we disallow redirects
  ...
  if ($sefConfig->Enabled) {
    ...
    _log('This site $live_site value : ' . $JConfig->getValue('config.live_site'));
    if (is_readable($sef404)) {
      ...
      if (isset ($_SERVER['REQUEST_URI'])) {
        ...
        $shPageInfo->URI = new sh_Net_URL((empty($shPageInfo->shHttpsSave)?JString::rtrim($GLOBALS['shConfigLiveSite'], '/'):$shPageInfo->shHttpsSave).'/'.ltrim($REQUEST,'/'));
      }else{
      ...
      if (strpos($GLOBALS['shConfigLiveSite'],$shPageInfo->URI->host) === false) {
        ...
        if (empty($sefConfig->shRewriteMode)) {
          ...
          $shPageInfo->baseUrl = $GLOBALS['shConfigLiveSite'] . JString::rtrim( $sefConfig->shRewriteStrings[$sefConfig->shRewriteMode], '/')
