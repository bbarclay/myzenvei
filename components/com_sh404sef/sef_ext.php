 ...
 * @version     $Id: sef_ext.php 1149 2010-01-25 11:39:24Z silianacom-svn $
 */
...
class sef_404
{
  ...
  // V 1.2.4.j shAppendString will be added to sef url, but not saved in DB or cache
  {
    ...
    if ((shFileExists(JPATH_ROOT.DS.'components'.DS.$option.DS.'router.php'))
    && ($shDoNotOverride                   // and param said do not override
    || (!$shDoNotOverride              // or param said override,
    && !shFileExists(sh404SEF_ABS_PATH.'components/com_sh404sef/sef_ext/'.$option.'.php')
    && !shFileExists(JPATH_ROOT.DS.'components/'.$option.'/sef_ext/'.$option.'.php'))))  {// but we don't have a plugin
    ...
    } else { // no plugin found at all, use Joomla 1.0.x sef
    ...
    if ($row) {
      ...
      if (empty($shPageInfo->autoRedirectsDisabled) && $sefConfig->redirectToCorrectCaseUrl) {
        ...
        if ($req != $row->oldurl) {
          ...
          $targetUrl = $shPageInfo->URI->protocol.'://'.$shPageInfo->URI->host.(!sh404SEF_USE_NON_STANDARD_PORT || empty($shPageInfo->URI->port) ? '' : ':'.$shPageInfo->URI->port)
  ...
  function getContentTitles($view,$id, $layout, $Itemid=0, $shLang = null, $sefConfig = null)  // V 1.2.4.q can force language
  {
