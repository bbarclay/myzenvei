 ...
 * @version     $Id: sh404sef.inc.php 1149 2010-01-25 11:39:24Z silianacom-svn $
 *
 */
...
switch ($shPageInfo->URI->path) {
  ...
  case $shPageInfo->base.$shPageInfo->index: {
    ...
    // is what JF will return)
      ...
      if ( empty($shPageInfo->autoRedirectsDisabled) && $sefConfig->shRedirectNonSefToSef && (!empty($shPageInfo->URI->url)) && strpos( $shPageInfo->URI->url, 'index2.php') === false && strpos( $shPageInfo->URI->url, 'tmpl=component') === false && empty($_POST)) {
