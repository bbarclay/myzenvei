...
class CTableStorageS3 extends JTable 
{
	...
	function store()
	{
		...
		if(!$isExist){
			...
			. ' SET `storageid`=' 	.$db->Quote($this->storageid)
			. ' , `resource_path`= '.$db->Quote($this->resource_path);
			$db->setQuery($query);
			$db->query();
			if($db->getErrorNum())
			{
			...
			. ' SET `resource_path`= '.$db->Quote($this->resource_path)
			. ' WHERE `storageid`='   .$db->Quote($this->storageid);
			$db->setQuery($query);
			$db->query();
			if($db->getErrorNum())
			{
