...
function getAutoSuggestInput($fieldId = 'autoSuggestUserName', $hiddenField = false, $hiddenFieldId = 'autoSuggestUserId', $width = 200, $minChar = 3, $delay = 400, $cacheLength = 10)
{
	...
	$onClickAction		= (!empty($hiddenField))? "onclick=\"jQuery(\'#$hiddenFieldId\').val('+data[1]+');\"" : "";		
	...
	$js =<<<SHOWJS
		jQuery().ready(function(){
			jQuery("#$fieldId").autocomplete("$autoCLink", {
				minChars:$minChar, 
				matchSubset:1, 
				delay: 200,
			...
			jQuery("#$fieldId").result(function(event, data, formatted) {
					if (data)
						jQuery("#$hiddenFieldId").val(data[1]);
			});
			jQuery("#$fieldId").val('');
			jQuery("#$hiddenFieldId").val('');
		});
SHOWJS;
