...
function calculate_commisions($root)
{
 ...
 // echo $descendants = ( (($right - $left) - 1) / 2 );
   ...
   // now, retrieve all descendants of the $root node  
   //echo "SELECT a.* FROM mlm_geneology_tree as a,jos_vm_user_info as b WHERE  b.user_id=a.userid and spon_parent_id='$root'";         
...
$result = mysql_query("SELECT a.* FROM mlm_geneology_tree as a,jos_vm_user_info as b WHERE  b.user_id=a.userid and spon_parent_id='$root' order by treeid desc Limit 0,1");  

  $i=0;

echo "row counter ". mysql_num_rows($result);	
   while (($row = mysql_fetch_array($result))) 
   {  
  if($row['level'] <=$loop)
   	{
