...
<script src="scripts/jquery_002.js" type="text/javascript" ></script> 
    ...
    $(document).ready(function(){		
		...
		$(".o1").jqDock({size:80,distance:54,coefficient:1.5,duration:500,align:'middle',labels:false,source:false,loader:null});
		$(".o2").jqDock({size:50,distance:54,coefficient:1.5,duration:500,align:'middle',labels:false,source:false,loader:null});
		$(".o3").jqDock({size:30,distance:30,coefficient:1.5,duration:500,align:'middle',labels:false,source:false,loader:null});
		
		$(".o4").jqDock({size:70,distance:30,coefficient:1.5,duration:500,align:'middle',labels:false,source:false,loader:null});
		
		$(".o5").jqDock({size:40,distance:30,coefficient:1.5,duration:500,align:'middle',labels:false,source:false,loader:null});		
...
.o1  img { margin: -30px; width:72px; height:96px;
...
.o2  img{ margin: -30px;  width:72px; height:96px;
...
.o3 img{ margin: -30px;  width:72px; height:96px;
...
.o4 img{ margin: -30px;  width:72px; height:96px;
...
.o5 img{ margin: -30px;  width:72px; height:96px;
...
function displayTree($root,$level,$position)
{
	...
	if( $level==0){
		$query2 = "SELECT avatar FROM  jos_community_users where userid = '$root'"; 
		...
		echo"<div  style=' position:absolute; top:140px; left:370px;' >";
			...
			$query3 = "SELECT avatar FROM  jos_community_users where userid = '$root_id'"; 
			$result3 = mysql_query($query3);
			$row3 = mysql_fetch_array($result3);
			$numrows3 =  mysql_num_rows($result3);		
			
			//echo 'asd'.$img=$row3['avatar'];
...
?>									  
	...
	<?php
		...
		$query2 = "SELECT avatar FROM  jos_community_users where userid = '$obj'"; 
		$result2 = mysql_query($query2);
		$row2 = mysql_fetch_array($result2);
		$user_img = $row2['avatar'];
		$user_img = 'images/default.jpg';
	?>  
		
	<a href="#"><img id="<?php $img_id++; echo $img_id; if ($level==2){$level2_list[]=$img_id;} else if($level==3){$level3_list[]=$img_id;} else if($level==4){$level4_list[]=$img_id; }?>" onmouseover="showUser(<?php echo $root_id; ?>)" onmouseout="blank()" src="<?php echo $user_img; ?>" style="width:72px; height:96px; <?php echo 'left:'.$cordx.'px;top:'.$cordy.'px;'; $cordx+=$tempx; $cordy+=$temp; ?>" /> </a>
	<?php
		}
	      else{
			  ?>
              <img id="<?php $img_id++; echo $img_id; if ($level==2){$level2_list[]=$img_id;} else if($level==3){$level3_list[]=$img_id;} else if($level==4){$level4_list[]=$img_id; }?>" src="images/empty.jpg" style="width:72px; height:96px; <?php echo 'left:'.$cordx.'px;top:'.$cordy.'px;'; $cordx+=$tempx; $cordy+=$temp; ?>" />
              <?php
...
function blank()
{
  ...
  //document.getElementById("txtHint").style.backgroundImage = "url(black_arrow)";  
}
...
<body>
<a href="http://www.myzenvei.com/index.php?option=com_mlm&amp;view=apps">Control Panel </a>
<!--<a href="http://localhost/Brandons/site/index.php?option=com_mlm&amp;view=apps">Control Panel </a> -->
<div id="txtHint" style="position:absolute;"  class="tooltip"><b>Person info will be listed here.</b></div>
		
<div style="height: 1000px; width: 1200px; position:absolute;" id="svgbasics">
</div>	
