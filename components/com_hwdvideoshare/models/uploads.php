...
class hwd_vs_uploads
{
	...
	if($_CONFIG_DATA['send_email_on_upload']){ emailUploadResults($_FILE_DATA, $_CONFIG_DATA, $_POST_DATA); }
		...
		if ($c->requiredins == 1) {
			...
			if (!copy($originals_Dir.$file_name, $base_Dir.$file_name)) {
	...
	global $database, $my, $acl, $mosConfig_absolute_path, $mosConfig_mailfrom, $mosConfig_fromname, $mosConfig_live_site, $Itemid, $mosConfig_sitename;
		...
		if (file_exists($file_video_path)) {
			...
			if ($c->requiredins == 1) {
				...
				if (!copy($originals_Dir.$file_video_name, $base_Dir.$file_video_name)) {
        			hwd_vs_tools::infomessage(4, 0, _HWDVIDS_TITLE_UPLDFAIL, _HWDVIDS_ERROR_UPLDERR06, "exclamation.png", 0);
					return;
				} else {
					if ($c->deleteoriginal == 1) {
	...
	global $database, $my, $acl, $mosConfig_absolute_path, $mosConfig_mailfrom, $mosConfig_fromname, $mosConfig_live_site, $Itemid, $mosConfig_sitename;
		...
		if (!isset($_FILES['upfile_0']['error'])) {
			...
			if ($c->requiredins == 1) {
				...
				if (!move_uploaded_file ($_FILES['upfile_0']['tmp_name'],$base_Dir.$file_video_name) || !JPath::setPermissions($base_Dir.$file_video_name)) {
        			hwd_vs_tools::infomessage(4, 0, _HWDVIDS_TITLE_UPLDFAIL, _HWDVIDS_ERROR_UPLDERR06, "exclamation.png", 0);
					return;
				}
				if ($c->aav == 1) {
	...
	{
		global $database, $mosConfig_live_site, $mosConfig_allowUserRegistration, $Itemid;
		$c = hwd_vs_Config::get_instance();
		$db = & JFactory::getDBO();
		$my = & JFactory::getUser();
		$acl= & JFactory::getACL();
