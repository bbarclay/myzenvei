...
class hwd_vs_GenerateThumbnail
{
	...
	function draw($path_base, $path_new, $filename_noext, $filename_ext, $thumb_position, $full_position = "0:00:00") {
		...
		if ( @!file_exists($path_sthumb) || (@filesize($path_sthumb) == 0) ) {
			$cmd_sinput = "$s->ffmpegpath -i $path_cmd_new -ss $thumb_position -t 00:00:01 -s ".$nthumbwidth."x".$nthumbheight." -r 1 -f mjpeg $path_cmd_sthumb -y";
			@exec("$sharedlib $cmd_sinput 2>&1", $cmd_soutput);
		}
		if ( @!file_exists($path_sthumb) || (@filesize($path_sthumb) == 0) ) {
			$cmd_sinput = "$s->ffmpegpath -i $path_cmd_new -ss $thumb_position -t 00:00:01 -s ".$nthumbwidth."x".$nthumbheight." -r 1 -f image2 $path_cmd_sthumb -y";
			@exec("$sharedlib $cmd_sinput 2>&1", $cmd_soutput);
		}
		if ( @!file_exists($path_sthumb) || (@filesize($path_sthumb) == 0) ) {
		...
		if ( @!file_exists($path_lthumb) || (@filesize($path_lthumb) == 0) ) {
			$cmd_linput = "$s->ffmpegpath -i $path_cmd_new -ss $thumb_position -t 00:00:01 -s ".$lthumbwidth."x".$lthumbheight." -r 1 -f mjpeg $path_cmd_lthumb -y";
			@exec("$sharedlib $cmd_linput 2>&1", $cmd_loutput);
		}
		if ( @!file_exists($path_lthumb) || (@filesize($path_lthumb) == 0) ) {
			$cmd_linput = "$s->ffmpegpath -i $path_cmd_new -ss $thumb_position -t 00:00:01 -s ".$lthumbwidth."x".$lthumbheight." -r 1 -f image2 $path_cmd_lthumb -y";
			@exec("$sharedlib $cmd_linput 2>&1", $cmd_loutput);
		}
		if ( @!file_exists($path_lthumb) || (@filesize($path_lthumb) == 0) ) {
			$thumb_position = "0:00:01";
		...
		if ( @!file_exists($path_dthumb) || (@filesize($path_dthumb) == 0) ) {
			$cmd_dinput = "$s->ffmpegpath -i $path_cmd_new -pix_fmt rgb24 -r 1 -loop_output 0 -ss 5 -t 10 -s ".$nthumbwidth."x".$nthumbheight." -f gif $path_cmd_dthumb -y";
