...
class hwd_vs_ConverterTools
{
	...
	function setupBatch($row, $type, $batch)
	{
		...
		while ($result = @mysql_fetch_array($query)) {
			...
			$sql1 = "UPDATE ".$row->dbprefix."hwdvidsvideos SET approved = 'converting_".$type."_".$batch."' WHERE video_id = '$filename_original'";
     ...
     * @param database A database connector object
     */
	function set($row, $filename_original)
	{
	...
	**/
	...
	{
		echo "<div class=\"box\"><div><h2>Finishing Conversion (File ".$n." of ".$count.")</h2></div>";

		echo "<div><b>UPDATING STATUS TO PENDING</b></div>";
		hwd_vs_ConverterTools::setPending($row, $filename_original);

		echo "<div><b>UPDATING VIDEO ID</b></div>";
		hwd_vs_ConverterTools::updateVideoId($row, $filename_original, $filename_noext);

		echo "<div><b>UPDATING VIDEO DURATION</b></div>";
		...
		echo "<div><b>CHECKING AUTOAPPROVAL SETTINGS</b></div>";
		hwd_vs_ConverterTools::autoApprove($row, $filename_noext);

		echo "<div><b>DELETING ORIGINAL</b></div>";
