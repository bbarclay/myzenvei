...
echo "<html><head><link type=\"text/css\" rel=\"stylesheet\" href=\"../../../administrator/components/com_hwdvideoshare/assets/css/converter.css\" /></head><body>";

// Restart converter
echo "<div class=\"box\"><div><h2>Re-start Converter</h2></div>
        <div style=\"padding:5px;\">
          <a href=\"../../../components/com_hwdvideoshare/converters/converter_internal.php\">
...
echo "<div class=\"box\"><div><h2>Connecting to Database</h2></div>";

$connectServer = hwd_vs_ConverterTools::connectServer($row);
if ($connectServer) {
	echo "<div class=\"success\">SUCCESS: Connected successfully to Joomla SQL server</div>";
} else {
...
if ($connectDatabase) {
	echo "<div class=\"success\">SUCCESS: Connected successfully to Joomla SQL database</div>";
} else {
...
if ($setupBatchCON) {
	echo "<div class=\"success\">SUCCESS: Successfully setup \"core\" batch conversion</div>";
} else {
...
if ($setupBatchTHU) {
	echo "<div class=\"success\">SUCCESS: Successfully setup \"flv\" batch conversion</div>";
} else {
    die ('Can\'t setup conversion : ' . $setupBatch);
}
...
if ($setupBatchSWF) {
	echo "<div class=\"success\">SUCCESS: Successfully setup \"swf\" batch conversion</div>";
} else {
    die ('Can\'t setup conversion : ' . $setupBatch);
}
...
if ($setupBatchMP4) {
	echo "<div class=\"success\">SUCCESS: Successfully setup \"mp4\" batch conversion</div>";
} else {
    die ('Can\'t setup conversion : ' . $setupBatch);
}
...
if ($setupBatchRD) {
	echo "<div class=\"success\">SUCCESS: Successfully setup \"re-generation\" batch conversion</div>";
} else {
    die ('Can\'t setup conversion : ' . $setupBatch);
}
...
$count = mysql_num_rows($selectBatch);
	...
	if ($count == 0) {
		echo "<div class=\"box\"><h2>There are no videos to convert</h2><h3>Exiting...</h3></div>";
	} else {
		...
		while ($result = mysql_fetch_array($selectBatch)) {
			...
			**/
			...
			if ($ConvertVideo[0] !== 2) {
				...
				echo "<div class=\"abortbox\"><h2>File ".$n." of ".$count." Aborted!</h2></div>";
			...
			**/
		    ...
		    hwd_vs_ConverterTools::finish($n, $count, $row, $filename_original, $filename_noext, $ExtractDuration[0], $ExtractDuration[1], $path_new_flv, $path_original);
			$file_contents = '['.date('Y-m-d H:i:s').'] [ Successful Video Conversion ] Video ID: '.$result['id'];
			hwd_vs_ConverterTools::writeLog($file_contents);

			echo "<div class=\"success\"><b>SUCCESS: All processes complete for file ".$n." of ".$count."</b></div>";
			$n = $n + 1;
			echo "</div>";
		}
	}
...
$count = mysql_num_rows($selectBatch);
	...
	if ($count == 0) {
		echo "<div class=\"box\"><h2>There are no videos waiting for thumbnail creation</h2><h3>Exiting...</h3></div>";
	} else {
	   /**
		...
		**/
		...
		while ($result = @mysql_fetch_array($selectBatch)) {
			...
			**/
			echo "<div class=\"box\"><div><h2>Copying FLV Video File (File ".$n." of ".$count.")</h2></div>";
				if(copy($path_original, $path_new_flv)){
					echo "<div class=\"success\">SUCCESS: Original video file copied to upload directory</div>";
				} else {
					echo "<div class=\"error\">ERROR: Could not copy the original video into the upload directory. Check directory permissions.</div>";
				}
			echo "</div>";
...
$count = mysql_num_rows($selectBatch);
	...
	// check if any waiting videos
	if ($count == 0) {
		echo "<div class=\"box\"><h2>There are no videos waiting for swf processing</h2><h3>Exiting...</h3></div>";
	} else {
	   /**
		* Process each video file
		**/
		...
		while ($result = @mysql_fetch_array($selectBatch)) {
			...
			**/
			echo "<div class=\"box\"><div><h2>Copying SWF Video File (File ".$n." of ".$count.")</h2></div>";
...
$count = mysql_num_rows($selectBatch);
	...
	// check if any waiting videos
	if ($count == 0) {
		echo "<div class=\"box\"><h2>There are no videos waiting for mp4 processing</h2><h3>Exiting...</h3></div>";
	} else {
	   /**
		* Process each video file
		**/
		...
		while ($result = @mysql_fetch_array($selectBatch)) {
			...
			**/
		    if ($c->reconvertflv == 1) {
				...
				echo "<div class=\"box\"><div><h2>Copying MP4 Video File (File ".$n." of ".$count.")</h2></div>";
			...
			**/
		    ...
		    hwd_vs_ConverterTools::finish($n, $count, $row, $filename_original, $filename_noext, $ExtractDuration[0], $ExtractDuration[1], $path_video, $path_original);
...
$count = mysql_num_rows($selectBatch);
	...
	// check if any waiting videos
	if ($count == 0) {
		echo "<div class=\"box\"><h2>There are no videos waiting for duration re-calculation</h2><h3>Exiting...</h3></div>";
	} else {
	   /**
		* Process each video file
		**/
		...
		while ($result = @mysql_fetch_array($selectBatch)) {
			...
			echo "<div class=\"box\"><div><h2>Re-calculating Duration (File ".$n." of ".$count.")</h2></div>";
				...
				if ($result['video_length'] == "0:00:02" || $ExtractDuration[0] !== "0:00:02") {
					echo "<div><b>ADDING NEW DURATION (".$ExtractDuration[0].")</b></div>";
...
$count = mysql_num_rows($selectBatch);
	...
	// check if any waiting videos
	if ($count == 0) {
		echo "<div class=\"box\"><h2>There are no videos waiting for thumbnail re-generation</h2><h3>Exiting...</h3></div>";
	} else {
	   /**
		* Process each video file
		**/
		...
		while ($result = @mysql_fetch_array($selectBatch)) {
			...
			echo "<div class=\"box\"><div><h2>Re-calculating Duration (File ".$n." of ".$count.")</h2></div>";
				...
				if ($result['video_type'] == "local" || $result['video_type'] == "mp4") {
					...
					echo "<div><b>TAKING NEW THUMBNAILS (".$result['thumb_snap'].")</b></div>";
					$path_new = $path_base . "/uploads/" . $filename_noext . ".flv";
				...
				if ($GenerateThumbnail[0] == 0) {
				  echo "<div><b>STATIC FFMPEG INPUT</b></div>";
				  echo "<div><textarea rows=\"3\" cols=\"50\" style=\"width:90%\">".$GenerateThumbnail[2]."</textarea></div>";
				  echo "<div><b>STATIC FFMPEG OUTPUT</b></div>";
				  echo "<div><textarea rows=\"3\" cols=\"50\" style=\"width:90%\">".hwd_vs_ConverterTools::processOutput($GenerateThumbnail[3])."</textarea></div>";
				}
				if ($GenerateThumbnail[1] == 0) {
				  echo "<div><b>DYNAMIC FFMPEG INPUT</b></div>";
				  echo "<div><textarea rows=\"3\" cols=\"50\" style=\"width:90%\">".$GenerateThumbnail[4]."</textarea></div>";
				  echo "<div><b>DYNAMIC FFMPEG OUTPUT</b></div>";
				  echo "<div><textarea rows=\"3\" cols=\"50\" style=\"width:90%\">".hwd_vs_ConverterTools::processOutput($GenerateThumbnail[5])."</textarea></div>";
				}
				if ($GenerateThumbnail[6] == 0) {
				  echo "<div><b>LARGE FFMPEG INPUT</b></div>";
				  echo "<div><textarea rows=\"3\" cols=\"50\" style=\"width:90%\">".$GenerateThumbnail[7]."</textarea></div>";
				  echo "<div><b>LARGE FFMPEG OUTPUT</b></div>";
				  echo "<div><textarea rows=\"3\" cols=\"50\" style=\"width:90%\">".hwd_vs_ConverterTools::processOutput($GenerateThumbnail[8])."</textarea></div>";
				}
