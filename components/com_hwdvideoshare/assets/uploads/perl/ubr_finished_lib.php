...
function getConfigData($_XML_DATA){
	...
	for($i = 0; $i < count($_XML_DATA[0]['child'][0]['child']); $i++){
...
function getpostData($_XML_DATA){
	...
	for($i = 0; $i < count($_XML_DATA[0]['child'][1]['child']); $i++){
		if(isset($_XML_DATA[0]['child'][1]['child'][$i]['name']) && isset($_XML_DATA[0]['child'][1]['child'][$i]['content'])){
			$key = $_XML_DATA[0]['child'][1]['child'][$i]['name'];
			$value = $_XML_DATA[0]['child'][1]['child'][$i]['content'];
...
function emailUploadResults($_FILE_DATA, $_CONFIG_DATA, $_POST_DATA){
	$_FILE_DATA_EMAIL = getFileDataEmail($_FILE_DATA, $_CONFIG_DATA);
...
////////////////////////////////////////////////////////
function getFileDataTable($_FILE_DATA, $_CONFIG_DATA){
	$file_list = '';
	$col = 0;

	$file_list = "<table cellpadding=\"1\" cellspacing=\"1\" width=\"800px\">\n";
...
///////////////////////////////////////////////////////
function getFileDataEmail($_FILE_DATA, $_CONFIG_DATA){
	$email_file_list = '';
	$end_char = "\n";
		...
		if($file_size > 0){
			if($_CONFIG_DATA['link_to_upload_in_email']){ $email_file_list .= "File Name: " . $_CONFIG_DATA['path_to_upload'] . $file_name . "     File Size: " . $formatted_file_size . $end_char; }
				...
				if($_CONFIG_DATA['unique_upload_dir']){
					$email_file_list .= 'File Name: ' . $_CONFIG_DATA['unique_dir'] . '/' . $file_name . "     File Size: " . $formatted_file_size . $end_char;
				}
