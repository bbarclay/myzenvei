...
defined( '_JEXEC' ) or die( 'Direct Access to this location is not allowed.' );
     ...
     */
    function myspaceComPrepareVideo($row, $flv_width=null, $flv_height=null)
	{
		...
		$c = hwd_vs_Config::get_instance();
			...
			if ($c->playlocal == "1") {
				$truepath = myspaceComPrepareFlvURL($row->video_id);
			} else {
				$truepath = '';
			}
			...
			if (!empty($truepath) && $c->playlocal == "1") {
				...
				$code = "<object width=\"".$flv_width."px\" height=\"".$flv_height."px\" ><param name=\"allowFullScreen\" value=\"true\"/><param name=\"wmode\" value=\"transparent\"/><param name=\"movie\" value=\"http://mediaservices.myspace.com/services/media/embed.aspx/m=".$videocode.",t=1,mt=video\"/><embed src=\"http://mediaservices.myspace.com/services/media/embed.aspx/m=".$videocode.",t=1,mt=video\" width=\"".$flv_width."\" height=\"".$flv_height."\" allowFullScreen=\"true\" type=\"application/x-shockwave-flash\" wmode=\"transparent\"></embed></object>";
     ...
     */
	function myspaceComPrepareThumb($option, $vid, $Itemid, $k, $width=null, $height=null, $class=null, $tooltip_data=null)
	{
		if (!isset($width)) {
			$width = $c->thumbwidth;
		}
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function myspaceComPrepareThumbURL($option, $vid)
	{
		$data = explode(",", $option);
		$thumbnail = @$data[1];
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function myspaceComPrepareVideoURL($option)
	{
			if (@explode(",", $option)) {
				$data = explode(",", $option);
				$videocode = $data[0];
			...
			$code = "http://vids.myspace.com/index.cfm?fuseaction=vids.individual&VideoID=".$videocode;
			return $code;
	}
    /**
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function myspaceComPrepareVideoEmbed($option, $vid, $Itemid)
	{
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function myspaceComPrepareFlvURL($option)
	{
		global $mosConfig_sitename, $mosConfig_live_site;
		$c = hwd_vs_Config::get_instance();
