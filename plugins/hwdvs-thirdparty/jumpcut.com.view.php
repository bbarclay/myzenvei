...
defined( '_JEXEC' ) or die( 'Direct Access to this location is not allowed.' );
     ...
     */
    function jumpcutComPrepareVideo($row, $flv_width=null, $flv_height=null)
	{
		...
		$c = hwd_vs_Config::get_instance();
			...
			$truepath = jumpcutComPrepareFlvURL($row->video_id);
			...
			if (!empty($truepath) && $c->playlocal == "1") {
				...
				$code = "<embed src=\"http://www.jumpcut.com/media/flash/jump.swf?id=".$videocode."&asset_type=movie&asset_id=".$videocode."&eb=1\" width=\"".$flv_width."\" height=\"".$flv_height."\" type=\"application/x-shockwave-flash\"></embed>";
				return $code;
			}
     ...
     */
	function jumpcutComPrepareThumb($option, $vid, $Itemid, $k, $width=null, $height=null, $class=null, $tooltip_data=null)
	{
		if (!isset($width)) {
			$width = $c->thumbwidth;
		}
		...
		$code = "<img src=\"".jumpcutComPrepareThumbURL($option, $vid)."\" border=\"0\" width=\"".$width."\" height=\"".$height."\" title=\"".$tooltip_data[1]." :: ".$tooltip_data[2]."\" class=\"".$class."\" />";
		return $code;
	}
    /**
     * Prepares the code to insert the third party thumbnail image
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function jumpcutComPrepareThumbURL($option, $vid)
	{
		$data = explode(",", $option);
		$thumbnail = @$data[1];
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function jumpcutComPrepareVideoURL($option)
	{
			if (@explode(",", $option)) {
				$data = explode(",", $option);
				$videocode = $data[0];
			...
			$code = "http://jumpcut.com/view?id=".$videocode;
			return $code;
	}
    /**
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function jumpcutComPrepareVideoEmbed($option, $vid, $Itemid)
	{
		...
		$c = hwd_vs_Config::get_instance();
			...
			$code = "<embed src=&#34;http://jumpcut.com/media/flash/jump.swf?id=".$videocode."&asset_type=movie&asset_id=".$videocode."&eb=1&#34; width=&#34;408&#34; height=&#34;324&#34; type=&#34;application/x-shockwave-flash&#34;></embed>";
			if ($c->embedreturnlink == 1) {
				$jconfig = new jconfig();
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function jumpcutComPrepareFlvURL($option)
	{
		global $mosConfig_sitename, $mosConfig_live_site;
		$c = hwd_vs_Config::get_instance();
