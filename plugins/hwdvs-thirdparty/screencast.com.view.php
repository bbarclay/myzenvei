...
defined( '_JEXEC' ) or die( 'Direct Access to this location is not allowed.' );
     ...
     */
    function screencastcomPrepareVideo($row, $flv_width=null, $flv_height=null, $autostart=null)
	{
		...
		$c = hwd_vs_Config::get_instance();
			...
			if ($c->playlocal == "1") {
				$truepath = screencastcomPrepareFlvURL($row->video_id);
			} else {
				$truepath = '';
			}
			...
			if (!empty($truepath) && $c->playlocal == "1") {
				...
				$watchvideourl = "http://www.screencast.com".$videocode;
				...
				if (function_exists('curl_init')) {
					...
					if (!empty($buffer)) {
						...
						preg_match('/thumb=(.*?).swf/', $buffer, $match);
				...
				$code = '<object width="'.$flv_width.'" height="'.$flv_height.'"><param name="movie" value="http://content.screencast.com'.$videocode.'/bootstrap.swf"></param><param name="quality" value="high"></param><param name="bgcolor" value="#FFFFFF"></param><param name="flashVars" value="'.$match[0].'&containerwidth='.$flv_width.'&containerheight='.$flv_height.'"></param><param name="allowFullScreen" value="true"></param><param name="scale" value="showall"></param><param name="allowScriptAccess" value="always"></param><param name="base" value="http://content.screencast.com'.$videocode.'/"></param><embed src="http://content.screencast.com'.$videocode.'/bootstrap.swf" quality="high" bgcolor="#FFFFFF" width="'.$flv_width.'" height="'.$flv_height.'" type="application/x-shockwave-flash" allowScriptAccess="always" flashVars="'.$match[0].'&containerwidth='.$flv_width.'&containerheight='.$flv_height.'" allowFullScreen="true" base="http://content.screencast.com'.$videocode.'/" scale="showall"></embed></object>';
     ...
     */
	function screencastcomPrepareThumb($option, $vid, $Itemid, $k, $width=null, $height=null, $class=null, $tooltip_data=null)
	{
		if (!isset($width)) {
			$width = $c->thumbwidth;
		}
		...
		$code = "<img src=\"".screencastcomPrepareThumbURL($option, $vid)."\" border=\"0\" width=\"".$width."\" height=\"".$height."\" title=\"".$tooltip_data[1]." :: ".$tooltip_data[2]."\" class=\"".$class."\" />";
		return $code;
	}
    /**
     * Prepares the code to insert the third party thumbnail image
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function screencastcomPrepareThumbURL($option, $vid)
	{
		$data = explode(",", $option);
		$thumbnail = @$data[1];
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function screencastcomPrepareVideoURL($option)
	{
			if (@explode(",", $option)) {
				$data = explode(",", $option);
				$videocode = $data[0];
			...
			$code = 'http://www.screencast.com'.$videocode;
			return $code;
	}
    /**
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function screencastcomPrepareVideoEmbed($option, $vid, $Itemid)
	{
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function screencastcomPrepareFlvURL($option)
	{
			$data = explode(",", $option);
			$truepath = '';
			return $truepath;
