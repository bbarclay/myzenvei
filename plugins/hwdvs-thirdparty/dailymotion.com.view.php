...
defined( '_JEXEC' ) or die( 'Direct Access to this location is not allowed.' );
     ...
     */
    function dailymotionComPrepareVideo($row, $flv_width=null, $flv_height=null)
	{
		...
		$c = hwd_vs_Config::get_instance();
			...
			if ($c->playlocal == "1") {
				$truepath = dailymotionComPrepareFlvURL($row->video_id);
			} else {
				$truepath = '';
			}
			...
			if (!empty($truepath) && $c->playlocal == "1") {
				...
				$code = "<object width=\"".$flv_width."\" height=\"".$flv_height."\"><param name=\"movie\" value=\"http://www.dailymotion.com/swf/".$videocode."&v3=1&related=0\"></param><param name=\"allowFullScreen\" value=\"true\"></param><param name=\"allowScriptAccess\" value=\"always\"></param><embed src=\"http://www.dailymotion.com/swf/".$videocode."&v3=1&related=0\" type=\"application/x-shockwave-flash\" width=\"".$flv_width."\" height=\"".$flv_height."\" allowFullScreen=\"true\" allowScriptAccess=\"always\"></embed></object>";
				return $code;
			}
     ...
     */
	function dailymotionComPrepareThumb($option, $vid, $Itemid, $k, $width=null, $height=null, $class=null, $tooltip_data=null)
	{
		if (!isset($width)) {
			$width = $c->thumbwidth;
		}
		...
		$code = "<img src=\"".dailymotionComPrepareThumbURL($option, $vid)."\" border=\"0\" width=\"".$width."\" height=\"".$height."\" title=\"".$tooltip_data[1]." :: ".$tooltip_data[2]."\" class=\"".$class."\" />";
		return $code;
	}
    /**
     * Prepares the code to insert the third party thumbnail image
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function dailymotionComPrepareThumbURL($option, $vid)
	{
		$data = explode(",", $option);
		$thumbnail = @$data[1];
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function dailymotionComPrepareVideoURL($option)
	{
			if (@explode(",", $option)) {
				$data = explode(",", $option);
				$videocode = $data[0];
			...
			$code = "http://www.dailymotion.com/video/".$videocode;
			return $code;
	}
    /**
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function dailymotionComPrepareVideoEmbed($option, $vid, $Itemid)
	{
		...
		$c = hwd_vs_Config::get_instance();
			...
			$code = "<object width=&#34;420&#34; height=&#34;259&#34;><param name=&#34;movie&#34; value=&#34;http://www.dailymotion.com/swf/".$videocode."&v3=1&related=0&#34;></param><param name=&#34;allowFullScreen&#34; value=&#34;true&#34;></param><param name=&#34;allowScriptAccess&#34; value=&#34;always&#34;></param><embed src=&#34;http://www.dailymotion.com/swf/".$videocode."&v3=1&related=0&#34; type=&#34;application/x-shockwave-flash&#34; width=&#34;420&#34; height=&#34;259&#34; allowFullScreen=&#34;true&#34; allowScriptAccess=&#34;always&#34;></embed></object>";
			if ($c->embedreturnlink == 1) {
				$jconfig = new jconfig();
     ...
     * @return       $code   the full third party thumbnail image tag
     */
	function dailymotionComPrepareFlvURL($option)
	{
		global $mosConfig_sitename, $mosConfig_live_site;
		$c = hwd_vs_Config::get_instance();
			...
			$url = "http://www.dailymotion.com/video/".$data[0];
			...
			if (function_exists('curl_init')) {
				...
				if (empty($buffer))	{
					...
					$pos_flv_search = strpos($buffer, '.addVariable("video", "');

					if ($pos_flv_search === false) {

						$truepath = null;

					} else if ($pos_flv_search) {

						$pos_flv_start = $pos_flv_search+23;
						$pos_flv_end = strpos($buffer, '%40%40', $pos_flv_start);

						if ($pos_flv_end === false) {

							$truepath = null;

						}

						$length = $pos_flv_end - $pos_flv_start;

						$truepath_flv = substr($buffer, $pos_flv_start, $length);
						$truepath_flv = strip_tags($truepath_flv);
						$truepath_flv = urldecode($truepath_flv);

						//$pos_hq_start = strpos($buffer, '%40%40spark-mini%7C%7C', $pos_flv_start);
						//$pos_hq_start = $pos_hq_start + 22;
						//$pos_hq_end = strpos($buffer, '%40%40', $pos_hq_start);
						//$length_hq = $pos_hq_end - $pos_hq_start;
						//$truepath_hq = substr($buffer, $pos_hq_start, $length_hq);
						//$truepath_hq = strip_tags($truepath_hq);
						//$truepath_hq = urldecode($truepath_hq);

						$truepath = $truepath_flv;
						$truepath = hwd_vs_tools::get_final_url( $truepath );
					}
				}
			}

			$proxycheck = strpos($truepath, 'proxy');

			if (isset($truepath) && !empty($truepath) && $proxycheck === false) {
