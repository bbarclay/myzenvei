...
* JCEMediaBox 1.0.5
*
* @version 		$Id: jcemediabox.js 533 2010-02-17 13:03:58Z happynoodleboy $
* @package 		JCEMediaBox
...
(function() {
	...
	JCEMediaBox.Popup = {
		...
		build : function(){
			...
			this.page = DOM.add(document.body, 'div', {id : 'jcemediabox-popup-page'});
			...
			// Create Frame
			this.frame = DOM.add(this.page, 'div', {id : 'jcemediabox-popup-frame'}, '<div id="jcemediabox-popup-body">' + this.popuptheme + '</div>');
		...
		listener: function(e){
			switch (e.keyCode){
				...
				case 37: this.previousItem(); break;	
				case 39: this.nextItem(); break;
			}
		},
			...
			JCEMediaBox.each(['top', 'bottom'], function(s){
				...
				if (el) {
					...
					JCEMediaBox.FX.animate(el, style, ss, function(){
						if (!changed) {
							changed = true;
							JCEMediaBox.FX.animate(t.content, {'opacity' : 0}, JCEMediaBox.options.popup.fadespeed, function(){
								return t.change(n);	
							});
						}
					});
		...
		info : function(){
	        ...
	        if (this.caption) {
				...
				// Lazy layout fix for IE6/7
				if ((JCEMediaBox.isIE6 || JCEMediaBox.isIE7) && !h) {
			...
			switch(this.active.type) {
				...
				case'divx':
					...
					if (flash || isIE) {
						...
						for (n in p){
							if(p[n] !== ''){
								if (/(id|name|width|height|style|type)$/.test(n)){
					...
					// Use embed for non-IE browsers
						...
						this.object = '<embed id="jcemediabox-popup-object"';
						for (n in p){
							if(p[n] !== ''){
								t.object += ' ' + n + '="' + decodeURIComponent(p[n]) + '"';	
							}
				...
				case 'text/xml':
					...
					styles = extend(this.styles(p.styles), {display : 'none'});
					// Create ajax container
					this.ajax = DOM.add(this.content, 'div', {id : 'jcemediabox-popup-ajax', 'style' : styles});
		...
		setup: function(){
			...
			if (this.active.type == 'image') {	
				...
				// Protect images from right-click context menu - disabled until I can get it working in Opera!
				/*if (JCEMediaBox.options.popup.protect == 1) {
					var img = DOM.get('jcepopup-img');
					Event.add(img, 'mousedown', function(e) {
						if (JCEMediaBox.isOpera) {
					...
					Event.add(img, 'contextmenu', function(e) {
						Event.cancel(e);
					});
				}*/			
			}
		...
		animate : function(){
			...
			// Animate width
			FX.animate(this.body, {'height' : ch, 'top' : top, 'width' : cw}, ss, function() {
				// Hide loader
				if (t.loader) {
					DOM.hide(t.loader);
				}
				...
				function showInfo() {
					...
					if (itop) {
						...
						FX.animate(itop, {'top' : 0}, ss,  function() {
							itop.style.zIndex = 0;
						});
					}
					...
					if (ibottom) {
						...
						FX.animate(ibottom, {'top' : 0}, ss,  function() {
				...
				if (JCEMediaBox.isIE && t.active.type == 'media') {
					DOM.style(t.content, 'opacity', 1);
					showInfo();
				} else {
					FX.animate(t.content, {'opacity' : 1}, fs, function() {
						showInfo();
					});
				}
			});
