	...
	*/
	...
	{			
		...
		if ($folders) {
			foreach ($folders as $folder) {
				$classes 	= array();
				$classes[] 	= is_writable(Utils::makePath($this->getBaseDir(), $folder['url'])) || $this->isFtp() ? 'writable' : 'notwritable';
		...
		if ($files) {
			foreach ($files as $file) {
				$classes 	= array();
				$classes[] 	= is_writable(Utils::makePath($this->getBaseDir(), $file['url'])) || $this->isFtp() ? 'writable' : 'notwritable';
