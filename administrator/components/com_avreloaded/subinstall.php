...
/**
 * SubInstaller for use in components
 *
 * @author Fritz Elfert
 * @licence GNU/GPLv2 http://www.gnu.org/licenses/gpl-2.0.html
 * @version $Id: subinstall.php 840 2008-04-30 04:13:07Z Fritz Elfert $
 */
class SubInstaller extends JObject {
    ...
    function _getExtID(&$e, $core) {
        ...
        case 'module':
            ...
            $query = 'SELECT id FROM #__modules WHERE module = "'.
                $name.'" AND iscore = '.$core.' AND client_id = '.
                $e->client.' GROUP BY module';
            break;
        case 'plugin':
            $query = 'SELECT id FROM #__plugins WHERE element = "'.
                $e->name.'" AND iscore = '.$core.' AND client_id = '.
        ...
        case 'component':
            ...
            $query = 'SELECT id FROM #__components WHERE `option` = "'.
                $name.'" AND iscore = '.$core;
            break;
        case 'template':
    ...
    function _setcore($id, $type, $lock) {
        ...
        switch ($type) {
        case 'module':
            $query = 'UPDATE #__modules SET iscore = '.$lock.' WHERE id = '.$id;
            break;
        case 'plugin':
            $query = 'UPDATE #__plugins SET iscore = '.$lock.' WHERE id = '.$id;
            break;
        case 'component':
            $query = 'UPDATE #__components SET iscore = '.$lock.' WHERE id = '.$id;
            break;
        }
        if ($query != null) {
            $this->_mainDb->setQuery($query);
