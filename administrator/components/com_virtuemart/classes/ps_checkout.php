...
----------------------------' 
    ...
    */
	function email_receipt($order_id) {
		...
		$dbbt = new ps_DB;
		$dbst = new ps_DB;		$feedbst = new ps_DB;
		$qt = "SELECT * FROM  jos_fixed_reg_fee WHERE userid='".$user_id."'";		$feedbst->query($qt);		$feedbst->next_record();		  
		...
		}		 	$shopper_reg_fixed_fee = $feedbst->f("regfee");		 
		...
		elseif (ORDER_MAIL_HTML == '1') {
			...
			// These are a lot of vars to import for the email confirmation			$order_total = $order_total + $shopper_reg_fixed_fee;
			$template->set_vars(array(
														...
														'order_tax' => $CURRENCY_DISPLAY->getFullValue($order_tax, '', $db->f('order_currency')). ps_checkout::show_tax_details( $db->f('order_tax_details'), $db->f('order_currency') ),														'regfee' => $CURRENCY_DISPLAY->getFullValue($shopper_reg_fixed_fee, '', $db->f('order_currency')),			
