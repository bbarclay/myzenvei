	...
	for ($i=0, $n=count( $jmenutype_names ); $i < $n; $i++)
		{
			...
			for ($c=0, $c_n=count( $menujdata ); $c < $c_n; $c++)
			{
				...
				$link = JRoute::_( 'index.php?option=com_jposition&controller=jposition_detail&task=edit&cid[]='.$menujdata[$c]->id );
	...
	for ($i_items=0, $n_items=count( $items ); $i_items < $n_items; $i_items++)
		{
			if ($items[$i_items]->position === $row)
			{
				...
				echo '<li class="edocs_li'.$prefix_published.'">'.$items[$i_items]->title.'</li>'; 
			}
		}
		?>
			</ul>	
