<?php
	...
	if(isset($_POST['payall']))
	{
		...
		$report_type = $_POST['weekly_monthly'];
		
		$query = "UPDATE mlm_generate_report SET payment_status = '$payment_status' "."
			 where status_date BETWEEN '$startdate' and '$enddate' ";
		$result=mysql_query($query);
		
		$query = "UPDATE reports SET weekly_monthly = '$report_type' "."
	  ...
	  /*
	  	if($_POST['st']==2)
		{
			$query="UPDATE mlm_generate_report SET payment_status='paid' 
			where status_date BETWEEN DATE_ADD(curdate(), INTERVAL -1 month) and curdate() ";
		}
		elseif($_POST['st']==1)
		{
			$query="UPDATE mlm_generate_report SET payment_status='paid'  
			where status_date BETWEEN DATE_ADD(curdate(), INTERVAL -7 DAY) and curdate() "; 
		}
...
function change_data(id,value)
{
	addTable('generate',value);

}
function addTable(id,val)
{
...
xmlhttp.open("GET","mlm_z_tableData.php?q="+1+"&q2="+L1+"&q3="+L2+"&q4="+L3+"&q5="+L4+"&q6="+L5+"&q7="+L6+"&q8="+L7+"&q9="+L8+"&q10="+M1+"&q11="+M2+"&q12="+M3+"&q13="+M4+"&q14="+M5+"&q15="+M6+"&q16="+M7+"&q17="+R1+"&q18="+R2+"&q19="+R3+"&q20="+R4+"&q21="+R5+"&q22="+R6+"&q23="+R7+"&q24="+R8+"&q25="+R9+"&q26="+R10+"&q27="+R11+"&q28="+R12+"&q29="+R13+"&q30="+R14+"&q31="+R15+"&q32="+R16+"&q33="+R17+"&q34="+val,true);
xmlhttp.send();
...
<div id="panel" >
<table  border="0"  cellpadding="5" cellspacing="5" style="margin-bottom:10px; margin-left:150px;" align="left"  >
  <tr>
...
<div id="apDiv5">
  <fieldset>
  <legend>
  <input type="image"  src = "media/images/generate.png"name="generate" id="generate" 
  value="Generate Report" class="btn-slide" onclick="addTable(id,0)"  />
  </legend>
  <div class="demo">
...
<p align="center">
  ...
  <p>
    <input type="checkbox" name="chkPendingAssoc" class="top5" id="chkPendingAssoc" checked="checked"/>    Show Pending Associates</p>
  <p>
    <input type="checkbox" name="chkActiveAssoc" class="top5"  id="chkActiveAssoc"  checked="checked"/>    Show Active Associates</p>
  <p>
    <input type="checkbox" name="chkInnactiveAssoc" class="top5"  id="chkInnactiveAssoc" checked="checked"/>    Show Innactive Associates</p>
  <p>
...
<script>
	...
	function makePayout(str)
	{
		//alert(str);	
		$("#payoutMade").load("paymentMade.php?id="+str, function(responseText) {
	  		$("#paymentMade").empty().append(responseText);
			$("#paymentMade").fadeOut();
		  });
