	...
	function runmaintenance()
	{
		...
		if ($run_permdel == 1) {
			...
			for($i=0, $n=count( $rows ); $i < $n; $i++) {
				$row = &$rows[$i];
					$path_flv = JPATH_SITE."/hwdvideos/uploads/".$row->video_id.".flv";
					$path_thumb = JPATH_SITE."/hwdvideos/thumbs/".$row->video_id.".jpg";
					if (@file_exists("$path_flv")) {
							@unlink("$path_flv");
					}
		...
		if ($run_archivelogs !== 0) {
			...
			$archivelogs_report = hwd_vs_logs::initiate($run_archivelogs);
		}
