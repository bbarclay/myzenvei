...
class hwd_vs_logs {
	...
	function initiate($override) {
		...
		hwd_vs_logs::archiveViews();
		hwd_vs_logs::archiveFavours();
		hwd_vs_logs::archiveVotes();
	...
	function archiveViews() {
		...
		if (count($rows) > 0) {
			...
			$mainframe->redirect( JURI::root( true ) . '/administrator/index.php?option=com_hwdvideoshare&task=runmaintenance&run_archivelogs=1' );
	...
	function archiveFavours() {
		...
		$db->SetQuery('SELECT * FROM #__hwdvidslogs_favours WHERE date <= DATE_SUB(NOW(),INTERVAL 30 DAY)');
		$rows = $db->loadObjectList();
	...
	function archiveVotes() {
		...
		$db->SetQuery('SELECT * FROM #__hwdvidslogs_votes WHERE date <= DATE_SUB(NOW(),INTERVAL 30 DAY)');
		$rows = $db->loadObjectList();
