...
function patch()
{
	jQuery( "#submit-patch" ).attr('disabled' , 'true');
	jQuery( "#submit-patch" ).html('Updating ...');
	
	jQuery( "#no-progress" ).css( 'display' , 'none' );
...
{
	jQuery( "#submit-privacy-patch" ).attr('disabled' , 'true');
	jQuery( "#submit-privacy-patch" ).html('Updating ...');
	
	jax.call('community' , 'admin,maintenance,ajaxPatchPrivacy' );
}
</script>
<h3 style="text-decoration: underline;"><?php echo JText::_('Maintenance');?></h3>
<fieldset style="width: 50%">
	<legend><?php echo JText::_('Update user privacy');?></legend>
	<div>Run this fix to update the user's privacy.</div>
	<button id="submit-privacy-patch" class="button" onclick="patchPrivacy();"><?php echo JText::_('Run Fix Now'); ?></button>
</fieldset>
<?php
if( !$this->isLatest )
{
...
<fieldset style="width: 50%">
	...
	<button id="submit-patch" class="button" onclick="patch();"><?php echo JText::_('Run Fix Now'); ?></button>
</fieldset>
<?php
}
?>
