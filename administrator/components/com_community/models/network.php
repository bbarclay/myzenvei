...
{	
	...
	function getJSNInfo()
	{
		...
		$table->load('config');
		$db_params = new JParameter( $table->params );
	...
	function saveImage( $file = '', $replace = 0 )
	{
		// Import libraries
		...
		{
			JError::raiseNotice(100, JText::_('CC ERROR IMAGE FILE ALREADY EXISTS'));
			return false;
		}
		...
		{
			JError::raiseWarning(100, JText::_('CC ERROR UNABLE TO UPLOAD FILE'));
			return false;
		}
	...
	function deleteImage( $path = null, $folder = ''  )
	{
		...
		{
			JError::raiseWarning(100, JText::_('CC UNABLE TO DELETE').htmlspecialchars($path, ENT_COMPAT, 'UTF-8').' '.JText::_('CC WARNFILENAME'));
		}
		...
		else if (is_dir($fullPath))
		{
			...
			{
				JError::raiseWarning(100, JText::_('CC UNABLE TO DELETE').$fullPath.' '.JText::_('CC NOT EMPTY'));
			}
		}
		return true;
	}
