 ...
 * @package     sh404SEF-15
 * @version     $Id: admin.sh404sef.php 1147 2010-01-23 18:22:02Z silianacom-svn $
...
switch ($task) {
  ...
  default: {
    include_once(JPATH_ADMINISTRATOR.'/components/com_sh404sef/404SEF_cpanel.php');
    displayCPanel();
    break;
  }
}
...
function viewSEF($option, $ViewModeId = 0)
{
  ...
  // select urls for a component

  // select urls for a task

  // select urls for a view
  ...
  if ( !empty($search) ) {  // V 1.2.4.q added search URL feature
    $searchTerm = $database->getEscaped( JString::trim( JString::strtolower( $search ) ) );
    $where .= empty( $where) ? '': ' AND (' . "oldurl  LIKE '%" . $searchTerm  . "%' OR "
...
function editSEF( $id, $option ) {
  ...
  $lists['shAliasList'] .= htmlspecialchars($alias->alias)."\n";
...
function viewMeta($option)
{
  ...
  if ( !empty($search) ) {  // V 1.2.4.q added search URL feature
    ...
    $database->getEscaped( JString::trim( JString::strtolower( $search ) ) ) . "%'";
  }
  // make the select list for the filter
...
function saveAliases($aliases, $newurl) {
  ...
  if (!empty($aliases[0])) {
    ...
    foreach($aliases as $alias) {
    		...
    		if (!empty($alias)) $values .=  '(\''.$newurl.'\', \''.htmlspecialchars_decode($alias).'\'),';
    }
...
function showConfig($option)
{
  ...
  $lists['joomla_live_site'] = empty( $lists['joomla_live_site']) ? _COM_SEF_TT_JOOMLA_LIVE_SITE_MISSING : $lists['joomla_live_site'];
...
function purge($option, $ViewModeId=0  ) {
  ...
  if ( !empty($confirmed)){
    ...
    switch ($total) {
      ...
      // we must still try to clear the cache, just in case it was not deleted
      // for some reason
      if (file_exists(JPATH_ROOT.'/components/com_sh404sef/cache/shCacheContent.php')) {
        unlink(JPATH_ROOT.'/components/com_sh404sef/cache/shCacheContent.php');
      }
...
function import_custom_CSV_OPEN_SEF($userfile) {  // V 1.2.4.t
  ...
  if (move_uploaded_file($userfile['tmp_name'], $uploadfile)) {
    ...
    foreach ($lines as $line){
      $line = JString::trim($line);
      $lineBits = explode(';', $line);
      $sefUrl = JString::ltrim(JString::trim($lineBits[2],'"'), '/');
      $nonSef = JString::trim($lineBits[3], '"');
