...
define('_COM_SEF_DISABLED',"<p class='error'>ПРИМЕЧАНИЕ: Поддержка SEF в Joomla выключена. Для использования SEF, пожалуйста, включите ее из страницы SEO<a href='".$GLOBALS['shConfigLiveSite']."/administrator/index.php?option=com_config'>Глобальная Конфигурация</a></p>");
...
define('_COM_SEF_TT_404PAGE','Статичная странца для 404 ошибок - Не найдено ');
...
define('_COM_SEF_TT_ENABLED','Если выбрано Нет, то будет использован стандартный SEF для Joomla');
...
define('_COM_SEF_TT_OLDURL','Только относительное перенаправление из Joomla каталога <i>без</i> слэш впереди');
...
define('_COM_SEF_TT_SH_REDIRECT_JOOMLA_SEF_TO_SEF', 'Если <strong>Да</strong>, то стандартные ссылки JOOMLA SEF будут перенаправлены командой 301 в sh404SEF эквивалент, если они есть в БД. Если их нет в БД, то они будут созданы налету. Warning: this feature will work in most cases, but may give bad redirects for some Joomla SEF URL. Leave off if possible.');
...
define('_COM_SEF_TT_SH_META_MANAGEMENT_ACTIVATED', 'Если <strong>Да</strong>, то META теги Title, Description, Keywords, Robots и Language будут управляться sh404SEF. Иными словами, оригинальные значения созданные Joomla и/или другими компонентами будут оставлены без изменений. ');
...
define('_COM_SEF_TT_SH_UAGENT_BLACK_LIST', 'Любой запрос, сделанный UserAgent строкой из этого списка, будет <strong>отклонен</strong>, с условием прохождения URL выше упоминаемых проверок. Вводить по одной UserAgent строке на линию (строку).');
