...
var NewCalendar = new Class({	
	...
	bounds: function(cal) {
		...
		// gwe tweak
		var start = new Date(this.options.range.min, 0, 1); // jan 1, 1000
		var end = new Date(this.options.range.max, 11, 31); // dec 31, 2999
	...
	caption: function(cal) {
		...
		if (this.options.navigation == 2) {
			caption.addClass('dualnav'); // gwe tweak
...
NewCalendar.implement(new Events, new Options);