...
//	JLoader::register('JApplication' , JPATH_LIBRARIES.DS.'joomla'.DS.'application'.DS.'application.php');
		...
		if(isset($_POST['Submit']))
		{
			...
			$uname_err = '';
			$err = false;
			$uname_sql = "SELECT `id` FROM jos_users WHERE username = 'aarti' LIMIT 1";
			//$uname_sql = "SELECT `id` FROM jos_users WHERE username = '".$_POST['username']."' LIMIT 1";
			$uname_res = $myzDb->query($uname_sql);
			$uname_row = $myzDb->fetchRow($uname_res);
			if(sizeof($uname_row) > 0) {
				$uname_err .= 'This username already exists. Please try with another username.<br />';
				$err = true;
			}

			$email_sql = "SELECT `id` FROM jos_users WHERE email = '".$_POST['email']."' LIMIT 1";
			$email_res = $myzDb->query($email_sql);
			$email_row = $myzDb->fetchRow($email_res);
			if(sizeof($email_row) > 0) {
				$uname_err .= 'This email already exists. Please try with another email.<br />';
				$err = true;
			}

			/*$rep_sql = "SELECT `id` FROM jos_jsusernames WHERE username = '".$_POST['replicated_siteid']."' LIMIT 1";
			$rep_res = $myzDb->query($rep_sql);
			$rep_row = $myzDb->fetchRow($rep_res);
			if(sizeof($rep_row) > 0) {
				$uname_err .= 'This Replicated Site ID already exists. Please try with another Replicated Site ID.';
				$err = true;
			}*/
