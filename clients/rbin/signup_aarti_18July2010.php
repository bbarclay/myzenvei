...
//	JLoader::register('JApplication' , JPATH_LIBRARIES.DS.'joomla'.DS.'application'.DS.'application.php');
		...
		{
			/* bof User Data Check Unique field*/
			$uname_sql = "SELECT `id` FROM jos_users WHERE username = '".$_POST['username']."' LIMIT 1";
			$uname_res = $myzDb->query($uname_sql);
			$uname_row = $myzDb->execute($uname_res);
			//$uname_row = $myzDb->mysql_fetch_array($uname_res);
			print_r($uname_row);
			die("test");
			$uname_cnt = $myzDb->num_rows($uname_res);
			if($uname_cnt > 0) {
				$uname_err = "This username already exists. Please try with another username.";
				//echo '<script type="text/javascript">document.getElementById().innerHTML='.$uname_err.'</script>';
				return;
				echo '<script type="text/javascript">window.location.href = "http://www.myzenvei.com/clients/signup_aarti.php?err=un"</script>';
			}

			$email_sql = "SELECT `id` FROM jos_users WHERE email = '".$_POST['email']."'";
			$email_res = $myzDb->query($email_sql);
			$email_cnt = $myzDb->num_rows($email_res);
			if($email_cnt > 0) {
				$email_err = "This email already exists. Please try with another email.";
				return;
				//echo '<script type="text/javascript">window.location.href = "http://www.myzenvei.com/clients/signup_aarti.php?err=em"</script>';
			}

			$rep_sql = "SELECT `id` FROM jos_jsusernames WHERE username = '".$_POST['replicated_siteid']."'";
			$rep_res = $myzDb->query($rep_sql);
			$rep_cnt = $myzDb->num_rows($rep_res);
			if($rep_cnt > 0) {
				$rep_err = "This Replicated Site ID already exists. Please try with another Replicated Site ID.";
				return;
			}
			/* eof User Data Check Unique field*/
			die("Aart;i");
				...
				$v_position = "M";
				 $result = "INSERT INTO mlm_geneology_tree 
							...
							values ( '$newuser_id' , '$rep_siteid' , '$left' , '$right'  
									...
									,  '1'  , '$rep_siteid' , '$shoppergroup', '$v_position',now(),0 )"; 
