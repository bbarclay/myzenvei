...
/**************************************************/
		...
		{	
			...
			#checking Payment for Mtrex
			$mtrex_variables["billing_first_name"] = $First_Name;
			$mtrex_variables["billing_last_name"] = $Last_Name;
			$mtrex_variables["billing_company"] = $Bussiness_Name;
			$mtrex_variables["billing_phone"] = $Day_Phone;
			$mtrex_variables["billing_address"] = $Shipping_Address1;
			$mtrex_variables["billing_city"] =$Shipping_city;
			$mtrex_variables["billing_state"] =$Shipping_state;
			$mtrex_variables["billing_zip"] =$Shipping_zip;
			...
			$mtrex_variables["billing_address"] = $_POST['add1_bill'];
			$mtrex_variables["billing_city"] =$_POST['city_bill'];
			$mtrex_variables["billing_state"] =$_POST['state_bill'];
			$mtrex_variables["billing_zip"] =$_POST['zip_bill'];
			$mtrex_variables["billing_country"] =	'USA';	
			*/
			
			$mtrex_variables["order_total"] =$TOTAL;
			$mtrex_variables["credit_card_name"] =$Name_on_Card;
			$mtrex_variables["credit_card_number"] =$Card_No;
			$mtrex_variables["credit_card_code"] =	'';	
			$mtrex_variables["expire_month"] =	$emonth;	
			$mtrex_variables["expire_year"] =	$eyear;	
			$Mtrexresult = $mtrex->process_payment($mtrex_variables,"YES");	
			$resArr = explode("=",$Mtrexresult);	
			//print_r($resArr);
			$aproveRes = explode("&", $resArr[4]);
			$transid = explode("&", $resArr[8]);
			...
			#saving order logs
			$myzDb->query("insert into orderlogs set orderlog='$ordlogs', transid='$transid[0]', ord_date=now(), userid='$newuser_id'");
...
?>				
		...
		alert("Thank you for your order. Your order has been successfully placed. A confirmation email has been sent to you.");
			...
			window.location.href = "http://www.myzenvei.com/index.php?option=com_community&view=frontpage&itemid=15";
...
<?php 
   ...
   {
   		$cardError = "Please try with new credit card, provided card number is not valid";
   }
}
ob_end_flush();
?>
...
<SCRIPT type="text/javascript">
	...
	/*$(document).ready(function()
	{
		...
		/*function checkEmail(params)
		{
			...
			$.ajax({  
				    type: "POST",  
				    url: "myzIsEmailExist.php",  
					data: "email="+ params,  
				    success: function(msg){
				     //alert(msg);
				    if(msg !="OK") 
					   { jQuery("#email_err").empty().append(msg);
					     jQuery("#email").val('');
						 document.getElementById("email").value='';
					     var st =  document.getElementById("email")
						 st.focus(); return false;				  
						} 
					   else
					    {
					    	jQuery("#email_err").fadeOut("slow");
					    }	
				 }
			});
		}*/
	...
	function verifyCreditCardOrder()
		{
		alert("submit");
		options=
			{
				url	:'singup_hameed.php',
				type:'POST',
...
<b><?php echo "<font color='red'>" .$cardError. "</font>"; ?></b>
<!--step1 starts -->
<div id="step1">
    ...
    <p>&nbsp;</p>
    <p><a href="signupnoregfee.php">Click here</a> to order products without creating an autoship order >></p>
    <p>&nbsp;</p>
...
<div id="step2">
    ...
    <table cellspacing="0" width="100%">
        ...
        <tr>
          <td>*Username</td>
          <td>
		  <input name="username"  type="text" id="username" onBlur="checkUsername(this.value)" value="<?php echo $_POST['username'] ?>" /> 
...
Replicated Site ID</td>
        <td><input name="replicated_siteid"  type="text" id="replicated_siteid" onBlur="checkRep(this.value)" value="<?php echo $_POST['replicated_siteid'] ?>" /></td>
        </tr>
        ...
        <tr>
          ...
          <td width="33%">
            <input name="fname" type="text" id="fname" value="<?php echo $_POST['fname'] ?>" />
          </td>
          <td width="17%">Last Name: </td>
          <td width="35%"><input name="lname" type="text" id="lname" value="<?php echo $_POST['lname'] ?>" /></td>
        </tr>
        <tr>
          <td>Birthday: </td>
        ...
        <tr id="buss_name_field">
          <td>Business Name: </td>
          <td colspan="3"><input name="buss_name" type="text" class="large" id="buss_name" value="<?php echo $_POST['buss_name'] ?>" /></td>
        </tr>		
        <tr>
        ...
        <tr>
          <td>Email: </td>
          <td><input name="email" type="text" id="email" onBlur="checkEmail(this.value)" value="<?php echo $_POST['email'] ?>" />
          <br />
<span id="email_err"></span></td>
          <td>Confirm Email: </td>
          <td><input name="c_email" type="text" id="c_email" value="<?php echo $_POST['c_email'] ?>" /></td>
        </tr>
        <tr>
          <td><span class="ssn_text">SSN/SIN:</span> </td>
          <td><input name="ssn" type="text" id="ssn" value="<?php echo $_POST['ssn'] ?>" /></td>
          <td>Confirm <span class="ssn_text">SSN/SIN:</span> </td>
          <td><input name="c_ssn" type="text" id="c_ssn" value="<?php echo $_POST['c_ssn'] ?>" /></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
        ...
        <tr>
          <td>Daytime Phone: </td>
          <td><input name="day_phone" type="text" id="day_phone" value="<?php echo $_POST['day_phone'] ?>" /></td>
          <td>Evening Phone: </td>
          <td><input name="even_phone" type="text" id="even_phone" value="<?php echo $_POST['even_phone'] ?>" /></td>
        </tr>
        <tr>
          <td>Cell Phone: </td>
          <td><input name="cell" type="text" id="cell" value="<?php echo $_POST['cell'] ?>" /></td>
          <td>Fax Number: </td>
        ...
        <tr>
          <td>First Name: </td>
          <td><input name="fname_co" type="text" id="fname_co" value="<?php echo $_POST['fname_co'] ?>" /></td>
          <td>Last Name: </td>
          <td><input name="lname_co" type="text" id="lname_co" value="<?php echo $_POST['lname_co'] ?>" /></td>
        </tr>
        <tr>
          <td>Birthday: </td>
          <td><input name="b_day_co" type="text" id="b_day_co" value="<?php echo $_POST['b_day_co'] ?>" /></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
...
<div id="step3">
    ...
    <table cellspacing="0" width="96%">
        ...
        <tr>
          ...
          <td colspan="3"><input name="add1_ship" type="text" class="large" id="add1_ship" value="<?php echo $_POST['add1_ship'] ?>" /></td>
        </tr>
        <tr>
          <td>Address 2: </td>
          <td colspan="3"><input name="add2_ship" type="text" class="large" id="add2_ship" value="<?php echo $_POST['add2_ship'] ?>" /></td>
        </tr>
        <tr>
          <td>City: </td>
          <td width="275"><input name="city_ship" type="text" id="city_ship" value="<?php echo $_POST['city_ship'] ?>" /></td>
          <td>State:</td>
        ...
        <tr>
          <td height="63">Zip:</td>
          <td><input name="zip_ship" type="text" id="zip_ship" value="<?php echo $_POST['zip_ship'] ?>" /></td>
          <td>Country: </td>
        ...
        <tr>
          <td>Address 1: </td>
          <td colspan="3"><input name="add1_bill" type="text" class="large" id="add1_bill" value="<?php echo $_POST['add1_bill'] ?>" /></td>
        </tr>
        <tr>
          <td>Address 2: </td>
          <td colspan="3"><input name="add2_bill" type="text" class="large" id="add2_bill" value="<?php echo $_POST['add2_bill'] ?>" /></td>
        </tr>
        <tr>
          <td>City: </td>
          <td><input name="city_bill" type="text" id="city_bill" value="<?php echo $_POST['city_bill'] ?>" /></td>
          <td>State:</td>
        ...
        <tr>
          <td>Zip:</td>
          <td><input name="zip_bill" type="text" id="zip_bill" value="<?php echo $_POST['zip_bill'] ?>" /></td>
          <td>Country: </td>
        ...
        <tr>
          <td colspan="3" class="bold" align="center">&nbsp;</td>
          <td></td>
          <td></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>Name on Card: </td>
          <td><input name="name_card" type="text" id="name_card" value="<?php echo $_POST['name_cardl'] ?>" /></td>
        </tr>
        <tr>
          <td>Card Number: </td>
          <td><input name="card_no" type="text" id="card_no" value="<?php echo $_POST['card_no'] ?>" maxlength="18" /></td>
...
/
	  <select id="expire_year" name="expire_year">
        ...
        <tr>
          ...
          <td><input name="csv" type="text" id="csv" value="<?php echo $_POST['csv'] ?>" maxlength="18" /></td>
          <td></td>
          <td></td>
        </tr>
    </table>
...
?>
    <h3>Step 4: Referring Associate Name or ID:
      <tr>
          <td width="129">
          <td colspan="3"><input name="Referre" type="text" class="large" id="Referre" value="<?php echo $refname;?>" <?php echo $dis; ?> readonly="readonly" /></td>
        </tr></h3>
</div>
<!--end step4 -->
