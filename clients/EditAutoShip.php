<?php 
	...
	if(count($_POST)>0)
	{
		...
		$transid  = $myzDb->fetch("select transid from orderlogs where userid = '$userid'");
		
		
		#update Mtrex store 
		$mtrex_variables["billing_country"] =	'USA';	
		    $mtrex_variables["shipping_country"] =	'USA';	
			$mtrex_variables["mp_OrderNumber"]= $transid;
			$mtrex_variables["order_total"] =$regFee;
			$mtrex_variables["credit_card_name"] =$name_card;
			$mtrex_variables["credit_card_number"] =$card_no;
			$mtrex_variables["credit_card_code"] =	'';	
			$mtrex_variables["expire_month"] =	$expire_date;	
			$mtrex_variables["expire_year"] =	$expire_year;
			...
			if($aproveRes[0]=="Approved")
			{
				...
				mysql_query("update orderlogs set transid ='$transid[0]' where userid = '$userid'");
			}	
			else 
			{
	 ...
	 <tbody>
	  ...
	  <tr>
       ...
       <tr>
	    ...
	    <td><?php echo date("m/d/Y",strtotime($regFeeDate));?></td>
	    <td>Next Payment Date</td>
	    <td><?php echo date("m/d/Y",strtotime($nextPaymentDate));?></td>
       </tr>
        <tr>
	    <td>Reg. Fee</td>
